...
class AvatarTrainer:
    ...
    def test(self):
        ...
        geo_renderer = None
        if view_setting == 'camera':
            item_0 = self.dataset.getitem(0, training = False,view_idx=self.opt['test']['render_view_idx'])
        else:
        ...
        if dataset_module  == "MvRgbDatasetCustomSMPLX":
            global_orient = cv.Rodrigues(global_orient.reshape(3,3))[0]
        else:
        ...
        for idx in tqdm(range(data_num), desc = 'Rendering avatars...'):
            ...
            if view_setting == 'camera':
                item = getitem_func(
                    idx,
                    view_idx=self.opt['test']['render_view_idx'],
                    training = False,
                    extr = extr,
                    intr = intr,
                    img_w = img_w,
